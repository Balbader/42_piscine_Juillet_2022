...
/*   Updated: 2022/07/24 14:17:31 by baalbade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_trim_final_word(char *word)
{
	...
	final_word = (char *)malloc(sizeof(char) * ft_strlen(word));
	i = 0;
	j = 0;
	while (word[i])
	{
		if (!(ft_check_char(word[i])))
			i++;
		while ((buf[i] >= 32 && buf[i] <= 47) || (buf[i] >= 58 && buf[i] <= 126))
		{
